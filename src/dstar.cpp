 ...
 */
float Dstar::keyHashCode(state u) { return (float)(u.k.first + 1193 * u.k.second); }
...
int Dstar::computeShortestPath()
{
  ...
  int k = 0;
  while ((!openList.empty()) && (openList.top() < (s_start = calculateKey(s_start))) ||
...
bool Dstar::replan()
{
  ...
  // (%f,%f)\n",res,openList.size(),openHash.size(),openList.top().k.first,openList.top().k.second, s_start.k.first,
